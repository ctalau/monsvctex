...
In conclusion if the initial delay bound is greater than the threshold, the publishing skew will increase slightly and if it is smaller, the latency will increase more abruptly. 
...
In this section we evaluated the throughput that can be achieved when using the proposed scheduling strategy: subperiod algorithm for local scheduling and randomization based global scheduling. We tried to vary the slot size and the initial delay bound in order to achieve good throughput and small publishing skew and latency. 

In the charts \ref{fig:skew_sim} and \ref{fig:latency_sim} we show the results of publishing the entire set of histograms in 6000 slots. We tried the slot sizes 10, 15, 20 and 33 milliseconds and the initial delay bound to be 50, 66, 75 and 80\% of the slot size. 
...
{\Large \bf COMMENT SOMETHING: The histograms would take 25s in they were published in a sequential manner while in the previous scenario we need 90 seconds to publish the histograms in reasonable latency and publishing skew conditions. So, the throughput is 27\%.}
